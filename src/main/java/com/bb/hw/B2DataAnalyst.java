...
public class B2DataAnalyst {
    ...
    public static void main(String[] args) throws B2Exception {
        ...
        try (final B2StorageClient client = B2StorageClientFactory.createDefaultFactory().create(args[0], args[1],
                USER_AGENT)) {
            /*
            ...
            String err = "B2Exception code: " + be.getCode() + ",  status: " + be.getStatus() + ", message: "
                    + be.getMessage();
            errWriter.println("--- Error: " + err);
            s_logger.error(err);
            System.exit(1);
     ...
     * 
     * @param client: the consumer of B2 storage server; bucketId: a string of
     *                bucket identity
    ...
    /**
     * A file is downloaded from B2 storage server, and the priority queue handle
     * sorting The cases with null fieldId, null fileName, or null file content are
     * not considered
     * 
     * @param client: the consumer of B2 storage server; version: a B2FileVersio
     *                object; queue: a priority queue
     * @throws B2Exception if there are any troubles.
     */
    public static void downloadFileContent(B2StorageClient client, B2FileVersion version,
            B2ContentMemoryWriterDelegator sink, Queue<FileObject> queue) throws B2Exception {
